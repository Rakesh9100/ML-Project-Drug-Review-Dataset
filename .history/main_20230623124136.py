...
result = seasonal_decompose(np.reshape(Y_train, (-1,)) , model='ad',period=12)
result.plot()
plt.show()
...
arima_model = ARIMA(X_train,Y_train,order=(1,1,3))
model = arima_model.fit()
print(model.summary())
mean_actual =Y_train.mean()
# Predict future values
forecasted_values = model.predict(start=X_test.index[0],end=X_test.index[-1])
tss = ((Y_train - mean_actual) ** 2).sum()
rss = ((Y_test- forecasted_values) ** 2).sum()
r_squared = 1 - (rss / tss)
print("r2 value of arima: ",r_squared)
#mae,nse,rmse
from sklearn.metrics import mean_absolute_error,mean_squared_error,mean_squared_log_error,r2_score
print("MAE: ",mean_absolute_error(Y_test,forecasted_values))
print("MSE: ",mean_squared_error(Y_test,forecasted_values))
print("RMSE: ",np.sqrt(mean_squared_error(Y_test,forecasted_values)))
